<?php
	...
	class API extends \API {
		...
		public function __construct() {
			$this->_name = 'game';
			$this->_version = '0.1.1';
			$this->_release = '2021-05-09';
			$this->_schema = new \Game\Schema();
			parent::__construct();
		}
		...
		### Query Tile List								###
		###################################################
		public function findTiles() {
			$response = new \HTTP\Response();
	
			# Initiate Tile List
			$tile_list = new \Game\TileList();
	
			# Find Matching Tiles
			$parameters = array();
			$tiles = $tile_list->find($parameters);
	
			# Error Handling
			if ($tile_list->error()) error($tile_list->error());
			else{
				$response->tile = $tiles;
				$response->success = 1;
			}
		...
		###################################################
		### Get Details regarding Specified Tile		###
		###################################################
		public function getTile() {
			...
			$tile = new \Game\Tile();
			if (isset($_REQUEST['code'])) $tile->get($_REQUEST['code']);
			elseif (isset($_REQUEST['id'])) $tile = new \Game\Tile($_REQUEST['id']);

			# Error Handling
			if ($tile->error) error($tile->error);
			elseif ($tile->id) {
		...
		public function addTile() {
			if (! $GLOBALS['_SESSION_']->customer->can('add tiles')) error("Permission Denied");
